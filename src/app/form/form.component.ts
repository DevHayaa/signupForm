...
export class FormComponent  {
   ...
   signupForm = new FormGroup({
    ...
    confirm : new FormControl ('',[Validators.required, Validators.maxLength(32)])
  ...
  passwordMatchValidator(): any {
    return (signupForm: FormGroup): { [key: string]: boolean } | null => {
      const password = signupForm.get('Password')?.value;
      const confirmPassword = signupForm.get('confirm')?.value;
      if (password !== confirmPassword) {
        signupForm.get('confirm')?.setErrors({ mismatch: true });
